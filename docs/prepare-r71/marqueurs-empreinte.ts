import { segmentCommun, type SegmentCommun } from "./empreinte-sonore.js";
 ...
 */
export function repereParEmpreinte(reference: Float64Array, temoins: Float64Array[]): RepereSonore | null {
  const trouves: SegmentCommun[] = [];
  for (const temoin of temoins) {
    const segment = segmentCommun(reference, temoin);
