...
- **Linux est configuré mais pas produit.** Le `.deb` et l'AppImage sont décrits, et le rassemblement des morceaux de VLC d'un système Debian est écrit — mais aucune machine Linux n'était disponible : ce chemin n'est **pas éprouvé**, et il est dit tel quel plutôt que présenté comme fini.
...
- **L'assemblage des paquets Linux, lui, demande une machine Linux**, et c'est dit avant de commencer plutôt qu'après cent mégaoctets téléchargés. Les deux raisons ont été mesurées, la seconde après avoir installé `fpm` sous Windows pour lever le doute : ce n'est **pas** l'outil qui manque — il s'installe et se lance. C'est qu'electron-builder construit la description d'un `.deb` avec un saut de ligne en dur, et qu'un fichier de commandes Windows ne peut pas porter un argument qui en contient : la ligne se coupe là, et fpm ne reçoit jamais les chemins à empaqueter. Retirer le synopsis n'y change rien, le saut de ligne restant en tête. L'AppImage, elle, bute sur le privilège de créer un lien symbolique.
